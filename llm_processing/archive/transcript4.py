...
class Transcript:
    def __init__(self, image_ref: str, prompt_name: str, time_started=None):
        ...
        self.models = self.get_models_used()
    ...
    def get_prompt_name_from_base(self):
        return [val for val in self.versions.values()][-1]["data"]["prompt name"]

    def get_models_used(self):
        return list(set([v["data"]["created by"] for v in self.versions.values() if not v["data"]["is user"]]))          
    ...
    def create_version(self, created_by, content, data, is_user=False, old_version_name="base", editing={}):
        ...
        if not old_version_name=="base" and self.is_same_user(created_by, old_version_name):
            new_version_name = self.get_latest_version_name(idx=0)
            old_version_name =self.get_latest_version_name(idx=1)
        else:    
            new_version_name = self.get_version_name(created_by)
        new_content_dict, old_notes = self.update_content(content, new_version_name)
        new_editing_dict = self.update_editing(editing, old_notes, created_by, new_version_name, old_version_name)    
        new_data_dict = self.update_data(data, created_by, old_version_name, editing)
        self.versions[new_version_name] = {"content": new_content_dict, "data": new_data_dict, "editing": new_editing_dict}
    ...
    def is_same_user(self, created_by, old_version_name):
        version_names = [k for k in self.versions.keys()]
        return self.versions[old_version_name]["data"]["created by"] == created_by

    def update_content(self, content, new_version_name):   
        content_dict, old_notes = {}, {}
        for fieldname, fieldvalue in content.items():
            old_notes[fieldname] = content[fieldname]["notes"] if "notes" in content[fieldname] else ""
            new_notes =  content[fieldname]["new_notes"] if "new_notes" in content[fieldname] else ""
            most_recent_notes = f"{new_version_name}: {new_notes}" if new_notes else old_notes[fieldname]
            value = content[fieldname]["value"] if "value" in content[fieldname] else fieldvalue
            content_dict[fieldname] = {"value": value, "notes": most_recent_notes, "new_notes": ""}
        return content_dict, old_notes

    def update_editing(self, editing, old_notes, created_by, new_version_name, old_version_name):
        for fieldname, old_note in old_notes.items():
            if fieldname not in editing:
                editing[fieldname] = [old_note]
            elif not old_note or old_note in editing[fieldname]:
                continue
            else:
                editing[fieldname] += [old_note]
        return self.get_history_dict(created_by, new_version_name, old_version_name) | editing     

    def get_history_dict(self, created_by, new_version_name, old_version_name):
        if old_version_name == "base":
            history = [(new_version_name, created_by)]
        else:
            history = self.versions[old_version_name]["editing"]["history"] + [(new_version_name, created_by)]
    ...
    def update_data(self, data, created_by, old_version_name, editing):
        d = {}
    ...
    def update_time_to_edit(self, current_version_name, start=True):
        if start:
            self.versions[current_version_name]["editing"] = {"time started": self.get_timestamp()}
        else:
            time_started = self.versions[current_version_name]["editing"]["time started"]
    ...
    def get_latest_version_name(self, idx=0):
        return self.sort_version_names_by_date(self.versions)[idx]     

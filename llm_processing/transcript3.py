...
class Transcript:
    ...
    def create_version(self, created_by, content, data, is_user=False, old_version_name="base", editing={}):
        ...
        print(f"in create_version: {old_version_name = }")
        if not old_version_name=="base" and self.is_same_user(created_by, old_version_name):
            ...
            print(f"{new_version_name = }")
            old_version_name =self.get_latest_version_name(idx=1)
            print(f"{old_version_name = }")
        else:    
            new_version_name = self.get_version_name(created_by)
        editing_dict = self.get_history_dict(created_by, new_version_name, old_version_name) | editing
        data = self.update_data(data, created_by, old_version_name, editing)
    ...
    def is_same_user(self, created_by, old_version_name):
        ...
        print(f"in is_same_user: {version_names = }")
        print(f"in is_same_user: {old_version_name = }")
    ...
    def update_data(self, data, created_by, old_version_name, editing):
        ...
        time_to_create = editing["time editing"] if editing else time.time() - time.mktime(time.strptime(self.time_started, "%Y-%m-%d-%H%M-%S"))

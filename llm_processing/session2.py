...
from llm_processing.run_manager import RunManager
...
#   
    def process_images(self, volume_name):
        if self.volume:
            self.volume.commit_volume()
        self.volume = self.initialize_volume(volume_name)
        self.pages = self.volume.pages
        self.run_manager = RunManager(self.msg, self.input_dict, self.user_name)
...
#
    ...
    def resume_jobs(self, try_failed_jobs=False):
        self.run_manager.resume_jobs(try_failed_jobs)
        transcript_pages = self.run_manager.process_images()
        for transcript_page in transcript_pages:
            self.volume.add_page(transcript_page)

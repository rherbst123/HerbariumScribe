    ...
    if current_chat and current_chat != st.session_state.chat_history:
        ...
        if new_message:
            try:
                ...
                processor = ClaudeImageProcessor(api_key, None, None)
...
def select_files(file_types=None, initial_dir=None):
    """
    Use Streamlit's file uploader for file selection.
    Returns a tuple of file paths or None.
    """
    # Convert file_types to accepted types for st.file_uploader
    if file_types:
        accepted_types = []
        for _, extensions in file_types:
            # Convert "*.png *.jpg *.jpeg" to ["png", "jpg", "jpeg"]
...
def process_images_callback(api_key_dict, prompt_text, selected_llms, 
                          selected_prompt_file, input_type, url_file, local_image_files):
    """Process images based on selected input type"""
    if input_type == "URL List":
        if url_file:
        ...
        if local_image_files:
            ...
            for uploaded_file in local_image_files:
                try:
                    image = Image.open(uploaded_file)
                    st.session_state.local_images.append((image, uploaded_file))
                except Exception as e:
                    st.warning(f"Could not open {uploaded_file}: {e}")
        else:
            st.error("No image files selected")
            return
...
def re_edit_saved_versions(selected_reedit_files):
    """Handle re-editing of saved version files"""
    save_edits_to_json()
    reset_processed_elements()
    reset_states()
    
    for uploaded_file in selected_reedit_files:
...
def re_edit_session(selected_session_file):
    """Handle re-editing of saved session file"""
    save_edits_to_json()
    reset_processed_elements()
    reset_states()
    
    try:
        json_str = selected_session_file.getvalue().decode('utf-8')

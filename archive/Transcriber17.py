...
def set_up():
    ...
    if "input_dict" not in st.session_state:
        st.session_state.input_dict = get_input_blank_dict()    
...
def chat_with_llm():
    if not st.session_state.show_chat_area:
        ...
        return
    api_key_dict = st.session_state.session_obj.input_dict["api_key_dict"]    
    if api_key_dict or "claude-3.5-sonnet" not in api_key_dict:
    ...
    if current_chat and current_chat != st.session_state.chat_history:
        ...
        if new_message:
            try:
                api_key = api_key_dict["claude-3.5-sonnet"].read().decode("utf-8").strip()
                processor = ClaudeImageProcessorThread(api_key, None, None)
                # use a selectbox to ask user if they want to include the url
                if st.session_state.include_image and st.session_state.session_obj.current_transcript_obj:
                    image = st.session_state.session_obj.processed_images[st.session_state.session_obj.current_image_index]
                    response, costs = processor.chat(new_message, image)
                else:
                    response, costs = processor.chat(new_message)
                st.session_state.session_obj.update_costs(costs)
...
def go_next_image():
    st.session_state.session_obj.go_next_image()
        
def go_previous_image():
    st.session_state.session_obj.go_previous_image()

def go_to_next_field():
    st.session_state.session_obj.go_to_next_field()
    
def go_to_previous_field():
    st.session_state.session_obj.go_to_previous_field()
...
def process_images_callback():
    #print(f"process_images_callback: {st.session_state.input_dict = }")
    selected_llms = st.session_state.input_dict["selected_llms"]
    selected_prompt_name = st.session_state.input_dict["selected_prompt_filename"]
    prompt_text = st.session_state.input_dict["prompt_text"]
    selected_images_info = st.session_state.input_dict["selected_images_info"]
    images_info_type = st.session_state.input_dict["images_info_type"]
    api_key_dict = st.session_state.api_key_dict
    # Reset states
...
def save_edits_as_text():
    st.session_state.session_obj.save_table_edits()
    ...
    for row_number, columns in edited_elements.items():
        for header, val in columns.items():
            if header in editables:
                st.session_state.session_obj.current_output_dict[fieldnames[row_number]][header] = val
...
def main():
    ...
    # ---------------
        ...
        #st.write(list(st.session_state.input_dict.values()))
                ...
                if st.session_state.pause_button_enabled:
                    with pause_button_col:
                        ...
                        if proceed_option == "Finish Remaining Jobs":
                            ...
                            msg = resume_jobs(try_failed_jobs=False)
                            ...
                            msg = resume_jobs(try_failed_jobs=True)
        ...
        with load_saved_edits_container:
            ...
            if loading_option == "Session":
                session_files = st.session_state.session_obj.get_session_files_list()
                if session_files:
                    ...
                    with col1:
                        if selected_session_file:
                            if st.button("Load Selected File"):
                                msg = re_edit_session(msg={}, selected_session_file)
                                st.session_state.reedit_mode = msg["reedit_mode"]
                    with col2:
                        if st.button("Cancel"):
                            st.session_state.reedit_mode = False
                            st.rerun()    
                    ...
                    with col1:
                        if selected_files:
                            if st.button("Load Selected Files"):
                                msg = st.session_state.session_obj.re_edit_saved_versions(msg={}, selected_files)
                                st.session_state.reedit_mode = msg["reedit_mode"]
                    with col2:
                        if st.button("Cancel"):
                            st.session_state.reedit_mode = False
    ...
    if st.session_state.session_obj.transcript_objs:
        st.session_state.session_obj.set_current_transcript_obj()
        ...
        with editor_container:
            ...
            with col_image:
                ...
                image = st.session_state.processed_images[current_image_idx]
                with col_button:
            ...
            with col_field_editor:
                ...
                st.session_state.session_obj.content_option = "content" if selected_content=="transcript" else selected_content
                ...
                if st.session_state.editing_view_option == "Fieldname":
                    ...
                    with text_container:
                        output_as_text = st.session_state.get_output_as_text_with_rating()
        ...
        with bottom_buttons_container:
            ...
            with col_save_to_json:
                st.button("Save edits to JSON", on_click=st.session_state.session_obj.save_edits_to_json)
            ...
            with col_output:
                ...
                st.text_area("Combined Output:", st.session_state.session_obj.final_output, height=600)
        ...
        st.write("No transcription to display.")    
    if st.session_state.fullscreen and st.session_state.session_obj.transcript_objs:

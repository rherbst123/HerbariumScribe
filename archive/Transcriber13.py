...
def save_table_edits(edited_elements, fieldnames, current_output_dict):
    ...
    for row_number, columns in edited_elements.items():
        print(f"row_number: {row_number}, columns: {columns}")
        #st.write(f"row_number: {row_number}, val: {val}")
        for header, val in columns.items():
            if header in editables:
                print(f"header: {header}, val: {val}")
    ...
    if st.session_state.processed_outputs and st.session_state.processed_images:
        ...
        with editor_container:
            ...
            with col_editor:
                ...
                if workspace_option == "fieldnames" or st.session_state.content_option != "content":
                    ...
                    with workspace_full_transcript_container:
                        if st.session_state.processed_outputs:
                            ...
                            validation_ratings = [str(get_validation_rating(fieldname)) for fieldname in fieldnames]
                            ...
                            edited_df = st.data_editor(df, use_container_width=False, column_config=column_config, hide_index=True, key="my_key", height=350)
                            edited_elements = st.session_state["my_key"]["edited_rows"]
                            st.button("Save", on_click=save_table_edits, args=(edited_elements, fieldnames, current_output_dict))
                        else:
                            st.write("No transcription to display.")

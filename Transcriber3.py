    ...
    with col_re_edit:
        if not st.session_state.reedit_mode:
            ...
            if reedit_files:
                ...
                with col1:
                    # Only show the Load button if files are selected
                    if selected_files:
                        if st.button("Load Selected Files"):
                            re_edit_callback(selected_files)
                with col2:
                    if st.button("Cancel"):
                        st.session_state.reedit_mode = False
                        st.rerun()
...
def re_edit_callback(selected_reedit_files):
    """Handle loading the selected file"""
    reset_states()
    for selected_file in selected_reedit_files:
        try:
            with open(os.path.join(f"{st.session_state.session_folder}versions", selected_file), "r", encoding="utf-8") as rf:
